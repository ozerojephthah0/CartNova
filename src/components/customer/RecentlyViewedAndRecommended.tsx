...
export const RecentlyViewedAndRecommended: React.FC<RecentlyViewedAndRecommendedProps> = ({
  ...
  return (
    <section className="bg-white dark:bg-slate-900 rounded-3xl p-5 sm:p-7 border border-slate-200/80 dark:border-slate-800 shadow-2xs space-y-6 my-6">
      {/* Header & Tabs */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-100 dark:border-slate-800 pb-5">
        <div>
          <div className="flex items-center gap-2 mb-1">
            {activeTab === 'recommended' && <Sparkles className="w-5 h-5 text-indigo-600 dark:text-indigo-400 animate-pulse" />}
            {activeTab === 'recently_viewed' && <Clock className="w-5 h-5 text-cyan-600 dark:text-cyan-400" />}
            {activeTab === 'trending' && <Flame className="w-5 h-5 text-rose-500 dark:text-rose-400" />}
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {showTabs && (
            <div className="inline-flex p-1 bg-slate-100/90 dark:bg-slate-800 rounded-2xl border border-slate-200/70 dark:border-slate-700 text-xs font-semibold">
              <button
              ...
              <button
                ...
                className={`flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl transition-all cursor-pointer relative ${
                  activeTab === 'recently_viewed'
                    ? 'bg-white dark:bg-slate-900 text-cyan-700 dark:text-cyan-300 shadow-2xs font-bold'
                    : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'
                }`}
              >
      ...
      {displayedList.length === 0 ? (
        <div className="py-12 text-center space-y-3 bg-slate-50/60 dark:bg-slate-800/40 rounded-2xl border border-dashed border-slate-200 dark:border-slate-800">
        ...
        <div className="overflow-x-auto pb-4 scrollbar-thin scrollbar-thumb-slate-200 dark:scrollbar-thumb-slate-800 flex gap-4 snap-x">
            ...
            <div key={product.id} className="min-w-[240px] max-w-[260px] shrink-0 snap-start relative group">
              {activeTab === 'recommended' && (
                <div className="mb-2 flex items-center justify-between text-[10px] font-bold">
                  <span className="px-2 py-0.5 bg-indigo-50 dark:bg-indigo-950/60 text-indigo-700 dark:text-indigo-300 rounded-md border border-indigo-200/60 dark:border-indigo-800">
                    {getMatchBadge(idx)}
                  </span>
                  <span className="text-slate-400 dark:text-slate-500">{getReasonTag(idx)}</span>
                </div>
              )}
              {activeTab === 'recently_viewed' && (
                <button
                  ...
                  className="absolute top-3 right-3 z-20 p-1.5 bg-white/90 dark:bg-slate-800/90 hover:bg-rose-50 dark:hover:bg-rose-950/50 text-slate-400 hover:text-rose-600 rounded-lg shadow-xs transition-colors cursor-pointer opacity-0 group-hover:opacity-100"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">
          {displayedList.map((product, idx) => (
            <div key={product.id} className="relative flex flex-col justify-between group">
              {activeTab === 'recommended' && (
                <div className="mb-2 flex items-center justify-between text-[10px] font-bold">
                  <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-indigo-50 dark:bg-indigo-950/60 text-indigo-700 dark:text-indigo-300 rounded-md border border-indigo-200/50 dark:border-indigo-800">
                    <Sparkles className="w-2.5 h-2.5" />
                    {getMatchBadge(idx)}
                  </span>
                  <span className="text-slate-400 dark:text-slate-500 truncate max-w-[130px]">{getReasonTag(idx)}</span>
                </div>
              )}
